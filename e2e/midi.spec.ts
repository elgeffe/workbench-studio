...
interface Sent { data: number[]; at: number }

// Web MIDI needs real hardware and a permission grant, so the port is stubbed:
// what we are testing is that the studio's transport addresses the K.O. II
// correctly — the right note numbers, on the right channel, with clock around
// them — not that Chromium can talk to USB.
  ...
  await page.addInitScript((names: string[]) => {
    ...
    names.forEach((name, i) => {
      outputs.set('port-' + i, {
        ...
        name,
        state: 'connected',
        send(data: number[], at?: number) { sent.push({ data: [...data], at: at ?? -1 }); },
...
test('the drum grid plays out as K.O. II pads, wrapped in clock and start/stop', async ({ page }) => {
  ...
  const noteOns = out.filter((m) => (m.data[0] & 0xf0) === 0x90);
...
test('a part is silent on the wire until its strip is switched on', async ({ page }) => {
  ...
  const out = await messages(page);
  const notes = out.filter((m) => (m.data[0] & 0xf0) === 0x90).map((m) => m.data[1]);
...
test('remapping a voice moves it to the pad you picked', async ({ page }) => {
  ...
  const notes = (await messages(page)).filter((m) => (m.data[0] & 0xf0) === 0x90).map((m) => m.data[1]);
